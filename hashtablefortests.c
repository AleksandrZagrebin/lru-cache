// ============================================================
// Хеш-таблица для тестирования (освобождает value)
// ============================================================
// Отличие от основной версии:
// - free_hash_node() освобождает value
// - hash_table_put() освобождает старое value при обновлении
// - Используется только в тестах для проверки утечек памяти
// ============================================================
...
// Освобождение узла (освобождает value)
...
static void hash_table_resize(struct HashTable_t* ht) {
    if (!ht) return;
...
// Освобождение хеш-таблицы (освобождает все value)
...
// При обновлении освобождает старое значение
void hash_table_put(struct HashTable_t* ht, int key, void* value) {
    ...
    while (current) {
        if (current->key == key) {
            // Ключ уже есть — освобождаем старое значение и обновляем
            free(current->value);
            current->value = value;
            return;
        }
...
// Удаление элемента по ключу (освобождает value)
...
// Очистка всех элементов (освобождает все value)
